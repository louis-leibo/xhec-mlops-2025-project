...
class AbaloneFeatures(BaseModel):
    ...
    length: float = Field(
        ...
        ge=0.0, 
        le=1.0, 
        description="Length of the abalone (mm)"
    )
    diameter: float = Field(
        ..., 
        ge=0.0, 
        le=1.0, 
        description="Diameter of the abalone (mm)"
    )
    height: float = Field(
        ..., 
        ge=0.0, 
        le=1.0, 
        description="Height of the abalone (mm)"
    )
    whole_weight: float = Field(
        ..., 
        ge=0.0, 
        description="Whole weight of the abalone (grams)"
    )
    shucked_weight: float = Field(
        ..., 
        ge=0.0, 
        description="Shucked weight of the abalone (grams)"
    )
    viscera_weight: float = Field(
        ..., 
        ge=0.0, 
        description="Viscera weight of the abalone (grams)"
    )
    shell_weight: float = Field(
        ..., 
        ge=0.0, 
        description="Shell weight of the abalone (grams)"
    )
...
class ModelInfoResponse(BaseModel):
    ...
    class Config:
        json_schema_extra = {
            "example": {
                "model_type": "LinearRegression with preprocessing",
